...
function listenArticleEve() {
	...
	$("#btnSb,.btn-adele").click(function() {
		...
		if (action !== "" && action === "delete") {
			...
			_param.article.deleted = 0;
			_param.article.createTime = new Date();
			//空值检测
			if(checkParm(_param)){
				//Ajax
				doAjax(_param);
	...
	function doAjax(_param) {
		...
		$.ajax({
			...
			success : function (res) {
				if (res == "SUCCESS") {
					alert(msgSucc);
					window.location.replace(contextPath+"/admin/articles");
				} else {
					alert("操作失败！");	
				} 
			}
	...
	$(".btn-aupd").click(function() {
		...
		window.location.href = contextPath + "/admin/article/init-edit?id="+aId;
	});
...
function listenTagEve() {
	...
	$(".delete").click(function() {
		_id = parseInt($(this).attr("tid"));
		$.post( contextPath+"/admin/tag/delete",{id:_id}, function(data) {
			if (data == "SUCCESS") {			
				alert(msgSucc);
				window.location.replace(contextPath+"/admin/tags");
			} else {
				alert(msgFail);
			}
		});
...
function listenFlinkEve() {
	...
	$(".flink-delete").click(function() {
		_id = $(this).attr("lId");
		$.get(contextPath + "/admin/frlink/delete", {id : _id}, function(res){
			if (res == "SUCCESS") {
				alert(msgSucc);
				window.location.replace(contextPath+"/admin/frlink");
			} else {
				alert(msgFail);
			}
		});
			...
			$.post(contextPath + reqUrl, {id:_id,name:_name}, function(data) {
				if (data == "SUCCESS") {
					if (action == "update") {//如果为更新操作
						...
						window.location.replace(contextPath+"/admin/categorys");
					}
				} else {
					$(".alert").html(msgFail);
				}
...
function listenAvatarUpoadEve()
{
	$("#avatarUploadBtn").click(function(){
	var formData = new FormData();
	formData.append('avatar', $('#avatarFile')[0].files[0]);
	formData.append('name',$('#avatarName').val());
	$.ajax({
	    url: contextPath+'/upload/avatar',
	    type: 'POST',
	    cache: false,
	    data: formData,
	    processData: false,
		...
		if(res == "SUCCESS"){
			$("#avatarUrl").val(contextPath+"/"+$('#avatarFile')[0].files[0].name);
		}
	...
	$("#updateUserBtn").click(function(){
		...
		$.ajax({
			url:contextPath+'/ucenter/update',
			type:'POST',
